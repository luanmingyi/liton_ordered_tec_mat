classdef TEC_ZONE_LOG < liton_ordered_tec.TEC_ZONE_BASE
    ...
    methods
        function obj = TEC_ZONE_LOG(varargin)
            ...
            elseif nargin==1
                ...
                if isa(varargin{1},'liton_ordered_tec.TEC_ZONE')
                    if isscalar(varargin{1})
                        ...
                        obj = repmat(liton_ordered_tec.TEC_ZONE_LOG,size(tec_zone_m));
                        for kk = 1:numel(tec_zone_m)
                            obj(kk) = liton_ordered_tec.TEC_ZONE_LOG(tec_zone_m(kk));
                        end
                    end
                elseif isa(varargin{1},'numeric') && isequal(mod(varargin{1},1),zeros(size(varargin{1})))
                    if isequal(mod(varargin{1},1),zeros(size(varargin{1})))
                        obj = repmat(liton_ordered_tec.TEC_ZONE_LOG,varargin{1});
                    else
